        ...
        <div className="w-full mx-auto ">
            ...
            <div className="flex   gap-1">
                <div className="mx-6 mt-4 shadow">
                    <button onClick={() => setShowCreate(true)} className="bg-white w-50 sm:max-w- flex flex-col h-48 items-center  justify-center  gap-2
            ...
            <div>
                {
                    showCreate && (
                        <form onSubmit={createResume} onClick={() => setShowCreate(false)} className="fixed items-center justify-center flex flex-col z-10 inset-0 backdrop-blur opacity-50 bg-black/100">
                            <div onClick={e => e.stopPropagation()} className="bg-white text-black-500 max-w-100 mx-4 md:p-6 p-4 text-left text-sm rounded shadow-[0px_0px_10px_0px] shadow-black/10 ">
            ...
            <div>
                {
                    upload && (
                        <form onSubmit={uploadResume} onClick={() => setUpload(false)} className="fixed items-center  justify-center flex flex-col z-10 inset-0 backdrop-blur opacity-50 bg-black/100">
                            <div onClick={e => e.stopPropagation()} className="bg-white text-black max-w-100 mx-4 md:p-6 p-4 text-left text-sm rounded shadow-[0px_0px_10px_0px] shadow-black/10 ">
                                ...
                                <label htmlFor="fileInput" className="border bg-white rounded-md text-sm w-80 border-indigo-600/60 p-8 flex flex-col items-center gap-4  cursor-pointer hover:border-indigo-500 transition">
                                    {resume ?
                                        <img width="44" height="44" src={resume} alt={resume} />
